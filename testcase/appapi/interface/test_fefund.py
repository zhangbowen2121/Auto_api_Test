...
class TestRefund(object):
    ...
    getBanksProvince_data = ExcelHandler().get_excel_data_1(file_path,"getBanksProvince")
    getBanksCity_data = ExcelHandler().get_excel_data_1(file_path,"getBanksCity")

    @pytest.mark.getBanksProvince
    @pytest.mark.parametrize('item',getBanksProvince_data)
    @allure.story('账户-退款-获取开户行信息-省份')
    def test_getBanksProvince(self,item,Pname):
        Iname='getBanksProvince'
        # 获取每一行数并且发请求：
    ...
    @pytest.mark.getBanksCity
    @pytest.mark.parametrize('item',getBanksCity_data)
    @allure.story('账户-退款-获取开户行信息-城市')
    def test_getBanksCity(self,item,Pname):
        Iname='getBanksCity'
        # 获取每一行数并且发请求：
        response = RequestHandler().get_response(item,Pname,Iname)
        # 行为驱动标记：
