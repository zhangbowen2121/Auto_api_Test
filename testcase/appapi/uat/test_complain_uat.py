...
@allure.feature('我要吐槽场景回归')
@pytest.mark.online
#@pytest.mark.uat
@pytest.mark.test
@pytest.mark.appapi
class TestComplainUat(object):
    # 测试的项目名称
    Pname = "appapi"
    # 测试用例数据获取路径
    DataFlie = Pname + "/" + ENV + "/uat"
    #测试的功能模块名称
    file_name = "complain_uat"

    #获取对应接口的测试数据
    file_path = ExcelHandler().get_excel_path(DataFlie,file_name)
    complain_data = ExcelHandler().get_excel_data_1(file_path,"complain")

    @pytest.mark.complain
    @pytest.mark.parametrize('item', complain_data)
    @allure.story('我要吐槽')
    def test_complain(self, item,Pname=Pname):
        # 获取每一行数并且发请求：
