...
@pytest.mark.online
@pytest.mark.appapi
class TestLessionUat(object):
    # 测试的项目名称
    Pname='appapi'
    # 测试用例数据获取路径
    DataFlie = Pname + "/" + ENV + "/uat"
    #测试的功能模块名称
    file_name = "lession_uat"

    #获取对应接口的测试数据
    file_path = ExcelHandler().get_excel_path(DataFlie,file_name)
    lession_data = ExcelHandler().get_excel_data_1(file_path,"lession")

    @pytest.mark.lession
    @pytest.mark.parametrize('item',lession_data)
    @allure.story('课件')
    def test_lession(self, item, Pname=Pname):
        # 获取每一行数并且发请求：
