...
@allure.feature('登录模块业务场景回归')
@pytest.mark.login
@pytest.mark.uat1
@pytest.mark.appapi
class TestLoginUat(object):
    # 测试的项目名称
    Pname = "appapi"
    # 测试用例数据获取路径
    ...
    login_data = ExcelHandler().get_excel_data_1(file_path,"login")

    @pytest.mark.loginByPwd
    @pytest.mark.parametrize('item', login_data)
    @allure.story('通过短信验证码登录，Token登录，退出登录')
    def test_loginByPwd(self, item,Pname=Pname):
        # 获取每一行数并且发请求：
