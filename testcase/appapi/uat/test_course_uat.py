...
@pytest.mark.online
@pytest.mark.appapi
class TestCourseUat(object):
    # 测试的项目名称
    Pname = "appapi"
    # 测试用例数据获取路径
    DataFlie = Pname + "/" + ENV + "/uat"
    #测试的功能模块名称
    file_name = "course_uat"

    #获取对应接口的测试数据
    file_path = ExcelHandler().get_excel_path(DataFlie,file_name)
    test_data = ExcelHandler().get_excel_data_1(file_path,"course")

    @pytest.mark.course
    @pytest.mark.parametrize('item', test_data)
    @allure.story('课节')
    def test_course(self, item,Pname=Pname):
        # 获取每一行数并且发请求：
        ...
        allure.dynamic.description(
            ...
            "<b style='color:red;'>实际执行结果：</b>{}<br />".format(
                ...
                getconf('api',ENV +'_url') + getconf('api',item['case_title']),
                response[0],
                response[1]
            ))
