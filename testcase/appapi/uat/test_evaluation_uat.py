...
@pytest.mark.online
@pytest.mark.appapi
class TestEvaluationUat(object):
    # 测试的项目名称
    Pname = "appapi"
    # 测试用例数据获取路径
    DataFlie = Pname + "/" + ENV + "/uat"
    #测试的功能模块名称
    file_name = "evaluation_uat"

    #获取对应接口的测试数据
    file_path = ExcelHandler().get_excel_path(DataFlie,file_name)
    evaluation_data = ExcelHandler().get_excel_data_1(file_path,"evaluation")

    @pytest.mark.evaluation
    @pytest.mark.parametrize('item',evaluation_data)
    @allure.story('评价')
    def test_evaluation(self, item,Pname=Pname):
        # 获取每一行数并且发请求：
