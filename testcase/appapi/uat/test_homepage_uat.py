...
@allure.feature('首页业务场景回归')
@pytest.mark.uat
@pytest.mark.test
@pytest.mark.online
@pytest.mark.appapi
@pytest.mark.homepage
class TestHomepageUat(object):
    # 测试的项目名称
    Pname = "appapi"
    # 测试用例数据获取路径
    DataFlie = Pname + "/" + ENV + "/uat"
    #测试的功能模块名称
    file_name = "homepage_uat"

    #获取对应接口的测试数据
    file_path = ExcelHandler().get_excel_path(DataFlie,file_name)
    test_data = ExcelHandler().get_excel_data_1(file_path,"homepage")

    @pytest.mark.homepage
    @pytest.mark.parametrize('item', test_data)
    @allure.story('首页')
    def test_homepage(self, item,Pname=Pname):
        # 获取每一行数并且发请求：
