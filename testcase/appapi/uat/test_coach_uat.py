...
class TestCoachUat(object):
    ...
    coach_data = ExcelHandler().get_excel_data_1(file_path,"coach")

    @pytest.mark.loginByPwd
    @pytest.mark.parametrize('item', coach_data)
    @allure.story('辅导方案')
    def test_coach(self, item,Pname=Pname):
        # 获取每一行数并且发请求：
